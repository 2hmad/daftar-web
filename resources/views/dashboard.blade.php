...
<body>
    ...
    <div class="got-gave" style="direction: rtl">
        <div>
            ...
            <span style="color: green" @if (LaravelLocalization::getCurrentLocale() == 'en') dir="ltr" @else dir="rtl" @endif>{{ $count_got }} {{ __('dashboard.egp') }}</span>
        </div>
        <div>
            <span>{{ __('dashboard.i-gave') }}</span>
            <span style="color: red" @if (LaravelLocalization::getCurrentLocale() == 'en') dir="ltr"
            @else dir="rtl" @endif>{{ $count_gave }} {{ __('dashboard.egp') }}</span>
        </div>
    </div>
            ...
            <a href="customer/{{ $customer->id }}">
                <div class="content">
                    <div class="circle" style="text-transform: uppercase">
                        {{ mb_substr($customer->customer_name, 0, 1, 'utf-8') }}</div>
    ...
    <button class="add-item" data-bs-toggle="modal" data-bs-target="#addItem"><i
                ...
                <form method="POST" action="{{ route('store-customer') }}">
                    <div class="modal-body" style="display: flex;flex-direction: column;gap: 15px;">
                        ...
                        <label>{{ __('dashboard.customer-name') }} @error('name')
                                <span class="error-feedback" style="color: red">( {!! $message !!}
                                )</span>@enderror</label>
